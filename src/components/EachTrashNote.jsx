...
function EachTrashNote({ note, func }) {
  ...
  return (
    ...
    >
      ...
      ></span>
      <button onClick={() => selectNotes(id)} className={`absolute top-0 left-0 z-2 grid size-[20px] -translate-1/2 cursor-pointer place-items-center transition-[color,opacity] duration-150 [@media(pointer:fine)]:group-hover:opacity-100 ${isSelected ? 'opacity-100' : 'opacity-0'}`}>
